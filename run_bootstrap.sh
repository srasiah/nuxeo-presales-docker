...
GIT_REPO_2021=$(grep '^GIT_REPO_2021' ${NUXEO_ENV_SECRET} | tail -n 1  | cut -d '=' -f2)

#rm -rf $NUXEO_NX_STUDIO

STUDIO_USERNAME=$NUXEO_STUDIO_USER CREDENTIALS="${NUXEO_STUDIO_PASSWORD}" IMAGE_TYPE=${NUXEO_IMAGE_TYPE} FQDN=${NUXEO_FQDN} NX_STUDIO=${NUXEO_NX_STUDIO} NX_STUDIO_VER=${NUXEO_NX_STUDIO_VER} WEBUI=${NUXEO_WEBUI} bash -c  "$(curl -fsSL https://raw.github.com/srasiah/nuxeo-presales-docker/lts2021/bootstrap.sh)" 